...
@Import(PostViewControllerWebTest.TestSecurity.class)
@ActiveProfiles("test")
class PostViewControllerWebTest {
	...
	static class TestSecurity {
		...
		org.springframework.security.web.SecurityFilterChain filter(
			...
			return http.authorizeHttpRequests(a -> a.requestMatchers(org.springframework.http.HttpMethod.GET, "/posts",
							"/posts/*", "/posts/new", "/posts/*/edit").permitAll().anyRequest().authenticated())
					.exceptionHandling(
							e -> e.authenticationEntryPoint((req, res, ex) -> res.sendRedirect("/register?required")))
					.build();
		}
	}
	...
	void 목록_200() throws Exception {
		...
		mvc.perform(get("/posts")).andExpect(status().isOk()).andExpect(view().name("posts/list"))
	...
	void 상세_200() throws Exception {
		...
		p.setTitle("t");
		p.setContent("c");
		p.setUser(new UserAccount());
		given(postService.findById(1L)).willReturn(p);
		given(attachmentRepository.findByPostIdOrderByIdAsc(1L)).willReturn(List.of());
	    given(commentService.list(1L)).willReturn(List.of());

		mvc.perform(get("/posts/1")).andExpect(status().isOk()).andExpect(view().name("posts/detail"))
	...
	void 작성폼_200() throws Exception {
		mvc.perform(get("/posts/new")).andExpect(status().isOk()).andExpect(view().name("posts/form"))
	...
	void 작성_성공_리다이렉트() throws Exception {
		...
		mvc.perform(post("/posts").with(csrf()).param("title", "T").param("content", "C"))
				.andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/posts/99"));
	}
	...
	void 수정폼_200() throws Exception {
		...
		mvc.perform(get("/posts/1/edit")).andExpect(status().isOk()).andExpect(view().name("posts/form"))
	...
	void 수정_PUT_성공_리다이렉트() throws Exception {
		...
		mvc.perform(put("/posts/1").with(csrf()).param("title", "T").param("content", "C").param("draftId", "d1"))
	...
	void 삭제_POST_delete_리다이렉트() throws Exception {
		...
		mvc.perform(post("/posts/1/delete").with(csrf())).andExpect(status().is3xxRedirection())
	...
	void 작성_비인증_로그인리다이렉트() throws Exception {
		mvc.perform(post("/posts").with(csrf()).param("title", "T").param("content", "C"))
				.andExpect(status().is3xxRedirection()).andExpect(redirectedUrl("/register?required"));
	}
}
